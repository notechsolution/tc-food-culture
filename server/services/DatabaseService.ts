...
import BaseEntity from '../models/BaseDefinition';

export default class DatabaseService<T extends BaseEntity> {
    ...
    async page(query, size, currentPage, populate = this.populate, sort?): Promise<PagedServiceResponse<T[]>> {
        try {
    ...
    /**
     * Finds rows with some given query,
     * return the results into the plain object => {},
     * like below without lean mode will return => { _doc: {}, otherProperties}
     * @param findParams Select parameters
     * @param {any[]} populate The population configurations used to join other tables
     * @returns {Promise<ServiceResponse>} All found rows
     */
    async findInLeanMode(findParams, populate = this.populate): Promise<T[]> {
        return await this.populateQuery(this.model.find(findParams).lean(), populate);
    }
    ...
    async updateById(id, body, getUpdatedModel = false): Promise<ServiceResponse<any>> {
        try {
            ...
            const data = (modelRes.data as unknown) as Document;
            const updateRes = await data.updateOne(body).exec();
            if (getUpdatedModel) {
